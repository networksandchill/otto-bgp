...
async def login(request: Request, data: LoginRequest):
    ...
    try:
        ...
        # Create tokens
        token_data = {
            'sub': username, 'role': user.get('role', 'read_only')}
...
async def refresh_token(request: Request):
    ...
    try:
        payload = jwt.decode(
            refresh_token, get_jwt_secret(), algorithms=[JWT_ALGORITHM])
        ...
        logger.info(
            f"200 refresh ok: issued new tokens for "
        ...
        return JSONResponse(
            {'error': 'Refresh token expired'}, status_code=401)
    except jwt_exceptions.InvalidTokenError:
        logger.info("401 refresh denied: invalid refresh token")
        return JSONResponse(
            {'error': 'Invalid refresh token'}, status_code=401)
