...
def _save_to_process_safe_cache(as_number: int, policy_content: str, policy_name: str = None, ttl: int = 3600):
    ...
    try:
        ...
        cache_data = {
            "cache_key": cache_key,
            "entry": {"data": policy_content, "timestamp": time.time(), "ttl_seconds": ttl, "key_hash": key_hash},
        }
...
class BGPq4Wrapper:
    ...
    DOCKER_IMAGES = {"default": "ghcr.io/bgp/bgpq4:latest", "alternative": "bgpq4/bgpq4"}
    ...
    def __init__(
        ...
        if proxy_active and self.mode in (BGPq4Mode.AUTO, BGPq4Mode.NATIVE):
            if self.detected_mode in (BGPq4Mode.DOCKER, BGPq4Mode.PODMAN):
                raise RuntimeError("IRR proxy requires native bgpq4. Containerized bgpq4 cannot reach host 127.0.0.1 tunnels.")

        proxy_status = 'enabled' if self.proxy_manager else 'disabled'
        cache_status = 'enabled' if self.enable_cache else 'disabled'
        self.logger.info(
            'BGPq4 wrapper initialized: mode=%s, command=%s, proxy=%s, cache=%s',
            self.detected_mode,
            self.bgpq4_command,
            proxy_status,
            cache_status,
    ...
    def _build_bgpq4_command(self, as_number: int, policy_name: str = None, irr_server: str = None) -> List[str]:
        ...
        if self.proxy_manager:
            ...
            self.logger.debug("Applied proxy configuration to bgpq4 command")
    ...
    def generate_policy_for_as(
        self, as_number: int, policy_name: str = None, irr_server: str = None, use_cache: bool = True, timeout: int = None
        ...
        try:
            ...
            result = run_with_resource_management(command=command, timeout=effective_timeout, capture_output=True, text=True)
    ...
    def generate_policy_for_object(
        self, object_name: str, policy_name: Optional[str] = None, timeout: Optional[int] = None
    ) -> PolicyGenerationResult:
        """
        ...
        try:
            ...
            error_msg = result.error_message or (
                f"bgpq4 error (code {result.returncode}): {result.stderr.strip()}" if hasattr(result, "returncode") else "bgpq4 error"
            )
            return PolicyGenerationResult(
                as_number=None,
                policy_name=name,
            ...
            return PolicyGenerationResult(
                as_number=None,
                policy_name=policy_name or object_name,
                policy_content="",
                success=False,
                execution_time=0.0,
                error_message=error_msg,
    ...
    def generate_policies_parallel(
        self, as_numbers: Union[List[int], Set[int]], custom_policy_names: Dict[int, str] = None, max_workers: int = None
    ) -> PolicyBatchResult:
        """
            ...
            try:
                with timeout_context(TimeoutType.BATCH_PROCESSING, f"parallel_generation_{len(validated_tasks)}_AS") as ctx:
                    with ProcessPoolExecutor(max_workers=max_workers) as executor:
                        # Submit all tasks
                        future_to_as = {executor.submit(_generate_policy_worker, task): task[0] for task in validated_tasks}
                ...
                remaining_tasks = [task for task in validated_tasks if not any(r.as_number == task[0] for r in results)]
    ...
    def test_bgpq4_connection(self, test_as: int = 7922) -> bool:
        ...
        try:
            ...
            if result.success and result.policy_content:
                self.logger.info(
                    f"bgpq4 test successful: generated {len(result.policy_content)} characters in {result.execution_time:.2f}s"
                )
                return True
            else:
    ...
    @classmethod
    def create_with_proxy(cls, proxy_config, mode: BGPq4Mode = BGPq4Mode.AUTO, command_timeout: int = 30):
        """
        ...
        return cls(mode=mode, command_timeout=command_timeout, proxy_manager=proxy_manager)
