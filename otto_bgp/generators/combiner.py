...
class StreamingDeduplicator:
    ...
    def deduplicate_prefixes_streaming(self, prefix_sources: List[Iterator[str]]) -> Iterator[str]:
        ...
        try:
            for i, prefix_source in enumerate(prefix_sources):
                try:
                    ...
                    with tempfile.NamedTemporaryFile(mode="w", delete=False, prefix=f"dedup_source_{i}_") as temp_file:
...
class PolicyCombiner:
    ...
    def combine_policies_for_router(
        self, router_hostname: str, policy_files: List[Path], output_dir: Path, format: str = "juniper"
        ...
        try:
            ...
            if use_streaming:
                return self._combine_policies_streaming(router_hostname, policy_files, output_file, format)
            else:
                return self._combine_policies_standard(router_hostname, policy_files, output_file, format)

        except Exception as e:
            self.logger.error(f"Failed to combine policies for {router_hostname}: {e}")
            return CombinedPolicyResult(
                router_hostname=router_hostname, policies_combined=0, output_file="", success=False, error_message=str(e)
            )

    def _combine_policies_streaming(
        self, router_hostname: str, policy_files: List[Path], output_file: Path, format: str
    ) -> CombinedPolicyResult:
        """
        ...
        try:
            ...
            self.logger.info(
                "Streaming combination complete. Memory peak: %.1fMB (final: %.1fMB)",
                self.memory_peak_mb,
                end_memory,
            )
    ...
    def _combine_policies_standard(
        self, router_hostname: str, policy_files: List[Path], output_file: Path, format: str
    ) -> CombinedPolicyResult:
        """
        ...
        for policy_file in policy_files:
            if policy_file.exists():
                ...
                as_number = self._extract_as_number_from_filename(policy_file.name)
                policies.append({"as_number": as_number, "content": content, "file": policy_file.name})
            else:
                self.logger.warning(f"Policy file not found: {policy_file}")
    ...
    def _write_juniper_format_streaming(
        self, output_file: TextIO, router_hostname: str, as_numbers: List[int], prefix_builder: StreamingPrefixListBuilder
    ) -> None:
    ...
    def _write_set_format_streaming(
        self, output_file: TextIO, router_hostname: str, as_numbers: List[int], prefix_builder: StreamingPrefixListBuilder
    ) -> None:
    ...
    def _write_hierarchical_format_streaming(
        self, output_file: TextIO, router_hostname: str, as_numbers: List[int], prefix_builder: StreamingPrefixListBuilder
    ) -> None:
    ...
    def _write_as_section_streaming(
        self, output_file: TextIO, as_number: int, prefix_builder: StreamingPrefixListBuilder, indent: int = 0
    ) -> None:
        ...
        for policy in policies:
            ...
            if prefix_list:
                ...
                if list_name not in prefix_lists:
                    prefix_lists[list_name] = {"as_number": as_number, "prefixes": []}
    ...
    def _extract_prefix_list(self, policy_content: str) -> Optional[Dict]:
        ...
        return {"name": list_name, "prefixes": prefixes}
    ...
    def merge_policy_directories(self, router_dirs: List[Path], output_dir: Path, format: str = "juniper") -> List[CombinedPolicyResult]:
        """
        ...
        for router_dir in router_dirs:
            ...
            if policy_files:
                result = self.combine_policies_for_router(
                    router_hostname=router_hostname, policy_files=policy_files, output_dir=output_dir, format=format
                )
