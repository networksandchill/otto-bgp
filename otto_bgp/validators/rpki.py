...
class ThreadHealthMonitor:
    ...
    def heartbeat(
        self, thread_id: str, operation_success: bool = True, timeout: bool = False
    ) -> None:
    ...
    def start_watchdog(
        self, check_interval: float = 10.0, max_silence: float = 30.0
    ) -> None:
        ...
        self.watchdog_thread = threading.Thread(
            target=self._watchdog_loop, args=(check_interval, max_silence), daemon=True
        )
    ...
    def _watchdog_loop(self, check_interval: float, max_silence: float) -> None:
        ...
        while self.watchdog_active:
            try:
                ...
                if unhealthy:
                    logger.warning(
                        f"Detected {len(unhealthy)} unhealthy threads: {unhealthy}"
                    )
...
class VRPEntry:
    ...
    def covers_prefix(self, test_prefix: str, test_asn: int) -> bool:
        ...
        try:
            ...
            return (
                (test_network.subnet_of(self.network) or test_network == self.network)
                and test_network.prefixlen <= self.max_length
                and test_asn == self.asn
            )
        except (AddressValueError, NetmaskValueError):
            return False
...
class StreamingVRPProcessor:
    ...
    def _stream_json_entries(self) -> Iterator[VRPEntry]:
        ...
        try:
            ...
            self.logger.warning(
                "ijson not available, falling back to non-streaming JSON parsing"
            )
                ...
                if b'"vrp_entries"' in file_content_sample:
                    ...
                    for roa in parser:
                        try:
                            ...
                            yield VRPEntry(
                                asn=int(
                                    str(roa["asn"]).replace("AS", "").replace("as", "")
                                ),
                                prefix=roa["prefix"],
                    ...
                    self.logger.warning(
                        "Unknown JSON format, attempting generic streaming"
                    )
                    yield from self._stream_json_entries_fallback()
                    ...
                    for roa in data["roas"]:
                        try:
                            yield VRPEntry(
                                ...
                                prefix=roa["prefix"],
                                max_length=roa.get(
                                    "maxLength", int(roa["prefix"].split("/")[1])
                                ),
                                ta=roa.get("ta", "unknown"),
                            )
                        except Exception as e:
                    ...
                    for vrp in data["validated-roa-payloads"]:
                        try:
                            yield VRPEntry(
                                ...
                                prefix=vrp["prefix"],
                                max_length=vrp.get(
                                    "max-length", int(vrp["prefix"].split("/")[1])
                                ),
                                ta=vrp.get("ta", "unknown"),
                            )
                        except Exception as e:
    ...
    def validate_prefix_streaming(self, prefix: str, asn: int) -> RPKIValidationResult:
        ...
        for vrp_entry in self.stream_vrp_entries():
            try:
                ...
                if (
                    target_network.subnet_of(vrp_network)
                    or target_network == vrp_network
                ):
...
class LazyVRPCache:
    ...
    def __init__(
        self,
        streaming_processor: StreamingVRPProcessor,
    ...
    def _can_cache_entry(self, vrp_entries: List[VRPEntry]) -> bool:
        ...
        if not isinstance(vrp_entries, list):
            raise TypeError(
                f"_can_cache_entry: vrp_entries must be a list, got {type(vrp_entries).__name__}"
            )

        if not vrp_entries:  # Empty list check
            self.logger.debug(
                "_can_cache_entry: vrp_entries is empty, nothing to cache"
            )
            return False

        if len(vrp_entries) == 0:  # Double-check for safety
            self.logger.debug(
                "_can_cache_entry: vrp_entries has zero length, nothing to cache"
            )
            return False
        ...
        if first_entry is None:
            self.logger.error(
                "_can_cache_entry: First VRP entry is None, cannot estimate cache size"
            )
            return False
        ...
        if not hasattr(first_entry, "__dict__"):
            self.logger.error(
                f"_can_cache_entry: First entry is not a valid VRP entry: {type(first_entry).__name__}"
            )
            return False
        ...
        try:
            ...
            if estimated_entry_size <= 0:
                self.logger.warning(
                    f"_can_cache_entry: Invalid memory estimate {estimated_entry_size}, using default"
                )
                estimated_entry_size = 1024  # Default reasonable size

            if estimated_entry_size > 1024 * 1024:  # 1MB per entry is suspicious
                self.logger.warning(
                    f"_can_cache_entry: Suspiciously large entry size {estimated_entry_size}, capping at 1MB"
                )
            ...
            if estimated_total_size < 0:
                self.logger.error(
                    "_can_cache_entry: Integer overflow in memory calculation, cannot cache"
                )
                return False
        ...
        if self._estimated_memory_usage + estimated_total_size > self.max_memory_bytes:
            ...
            if (
                self._estimated_memory_usage + estimated_total_size
        ...
        total_cached_entries = sum(
            len(entries) for entries in self._prefix_cache.values()
        )
        total_cached_entries += sum(
            len(entries) for entries in self._asn_cache.values()
        )
    ...
    def _evict_lru_entries(self):
        ...
        while (
            len(self._prefix_cache) + len(self._asn_cache)
        ) > self.max_cache_entries * 0.8:
            evicted = False
...
class RPKIValidator:
    ...
    def __init__(
        ...
        """
        self.logger = logger or logging.getLogger(__name__)
        self.vrp_cache_path = vrp_cache_path or Path(
            "/var/lib/otto-bgp/rpki/vrp_cache.json"
        )
        self.allowlist_path = allowlist_path or Path(
            "/var/lib/otto-bgp/rpki/allowlist.json"
        )
        ...
        self._streaming_processor = StreamingVRPProcessor(
            cache_path=self.vrp_cache_path, chunk_size=chunk_size, logger=self.logger
        )
    ...
    def _validate_prefix_streaming(self, prefix: str, asn: int) -> RPKIValidationResult:
        ...
        if SQLITE_AVAILABLE:
            try:
                ...
                if override_mgr.is_rpki_disabled(asn):
                    self.logger.info(
                        f"AS{asn}: RPKI validation disabled (customer override)"
                    )
                    return RPKIValidationResult(
                        prefix=prefix,
                        asn=asn,
        ...
        self.logger.debug(
            f"RPKI streaming validation: {prefix} AS{asn} -> {result.state.value} ({result.reason})"
        )
        return result
        ...
        if SQLITE_AVAILABLE:
            try:
                ...
                if override_mgr.is_rpki_disabled(validated_asn):
                    self.logger.info(
                        f"AS{validated_asn}: RPKI validation disabled (override)"
                    )
                    return {
                        "has_valid_roas": True,
                        "total_roas": 0,
    ...
    def validate_policy_prefixes(
        self, policy: Dict[str, Any]
    ) -> List[RPKIValidationResult]:
        """
        ...
        try:
            ...
            if not as_number:
                self.logger.warning(
                    "Policy missing AS number - skipping RPKI validation"
                )
                return results
    ...
    def validate_prefixes_parallel(
        self, prefixes: List[str], asn: int, max_workers: Optional[int] = None
    ) -> List[RPKIValidationResult]:
        """
        ...
        if len(prefixes) <= 10:
            self.logger.debug(
                f"Using sequential validation for {len(prefixes)} prefixes (small dataset)"
            )
            return [self.validate_prefix_origin(prefix, asn) for prefix in prefixes]

        # Parallel processing for larger datasets
        self.logger.debug(
            f"Using parallel validation for {len(prefixes)} prefixes with {max_workers} workers"
        )
        ...
        try:
            with timeout_context(
                TimeoutType.RPKI_VALIDATION, f"rpki_parallel_{len(prefixes)}_prefixes"
            ) as ctx:
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for i, chunk in enumerate(prefix_chunks):
                        ...
                        future = executor.submit(
                            self._validate_prefix_chunk_with_monitoring,
                            chunk,
                            asn,
                            thread_id,
                            health_monitor,
                    ...
                    while (
                        len(completed_futures) < len(future_to_chunk)
                        and not ctx.check_timeout()
                    ):
                        try:
                            # Calculate remaining timeout
                            timeout_remaining = min(
                                thread_timeout, ctx.remaining_time()
                            )
                            ...
                            pending_futures = (
                                set(future_to_chunk.keys()) - completed_futures
                            )
                            ...
                            for future in as_completed(
                                pending_futures, timeout=timeout_remaining
                            ):
                                ...
                                try:
                                    ...
                                    self.logger.debug(
                                        f"Completed chunk {chunk_index} ({len(chunk)} prefixes)"
                                    )

                                except FuturesTimeoutError:
                                    # Individual thread timeout
                                    health_monitor.mark_thread_failed(
                                        thread_id, "timeout"
                                    )
                                    self.logger.warning(
                                        f"Thread timeout for chunk {chunk_index}"
                                    )
                                    error_results = [
                                        RPKIValidationResult(
                                            prefix=prefix,
                            ...
                            if ctx.check_timeout():
                                self.logger.warning(
                                    "Overall RPKI validation timeout reached"
                                )
                                break
                    ...
                    if remaining_futures:
                        self.logger.warning(
                            f"Cancelling {len(remaining_futures)} incomplete futures due to timeout"
                        )
    ...
    def validate_policy_prefixes_parallel(
        self, policy: Dict[str, Any], max_workers: Optional[int] = None
    ) -> List[RPKIValidationResult]:
        """
    ...
    def _calculate_optimal_chunk_size(
        self, total_prefixes: int, max_workers: int
    ) -> int:
        """
    ...
    def _chunk_prefixes(self, prefixes: List[str], chunk_size: int) -> List[List[str]]:
        ...
        """
        return [
            prefixes[i:i + chunk_size] for i in range(0, len(prefixes), chunk_size)
        ]

    def _validate_prefix_chunk_with_monitoring(
        self,
        prefix_chunk: List[str],
        asn: int,
        thread_id: str,
    ...
    def _validate_prefix_chunk(
        self, prefix_chunk: List[str], asn: int
    ) -> List[RPKIValidationResult]:
        """
    ...
    def load_vrp_data(self, vrp_file_path: Path, source_format: str = "auto") -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(dataset.vrp_entries)} VRP entries from {source_format} format"
            )
            return True
    ...
    def update_allowlist(self, prefix: str, asn: int, add: bool = True) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Allowlist {action}: {sanitized_prefix} AS{sanitized_asn}"
            )
            return True
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        if vrp_entries_exist:
            try:
                ...
                vrp_data_stale = vrp_age_seconds > int(self.max_vrp_age_hours * 3600)
            except Exception:
                vrp_data_stale = True
    ...
    def _calculate_memory_reduction(
        self, vrp_count: int, actual_memory_mb: float
    ) -> float:
    ...
    def get_memory_pressure_report(self) -> Dict[str, Any]:
        ...
        memory_usage_percent = (
            cache_stats["estimated_memory_usage_mb"] / cache_stats["memory_limit_mb"]
        ) * 100
        ...
        if hit_rate < 60:
            ...
            recommendations.append(
                "Excellent cache hit rate - current strategy is effective"
            )

        # Add eviction analysis
        evictions = (
            cache_stats["cache_performance"]["evictions"]
            if "cache_performance" in cache_stats
            else 0
        )
        if evictions > 0:
            recommendations.append(
                f"Cache evictions detected ({evictions}) - consider increasing memory limit"
            )
        ...
        if invalid_length_vrps:
            return RPKIValidationResult(
                ...
                reason=(
                    ...
                    f"{target_network.prefixlen} exceeds max-length "
                    f"{invalid_length_vrps[0].max_length} of covering VRP "
                    f"{invalid_length_vrps[0].prefix}"
                ),
    ...
    def _load_allowlist(self):
        ...
        try:
            if not self.allowlist_path.exists():
                self.logger.info(
                    "No allowlist file found - starting with empty allowlist"
                )
                return
    ...
    def _save_allowlist(self):
        ...
        try:
            ...
            allowlist_data = {
                "entries": [
                    {"prefix": prefix, "asn": asn} for prefix, asn in self._allowlist
                ],
...
class RPKIGuardrail(GuardrailComponent):
    ...
    def check(self, context: Dict[str, Any]) -> GuardrailResult:
        ...
        for policy in policies:
            ...
            validation_results = self.rpki_validator.validate_policy_prefixes_parallel(
                policy
            )
        ...
        if invalid_percent > thresholds["max_invalid_percent"]:
            issues.append(
                f"{invalid_count} invalid prefixes ({invalid_percent:.1f}% > {thresholds['max_invalid_percent']}%)"
            )
            risk_level = "critical"

        # Check NOTFOUND threshold (excluding allowlisted)
        effective_notfound = notfound_count - allowlisted_count
        effective_notfound_percent = (
            (effective_notfound / total_prefixes) * 100 if total_prefixes > 0 else 0
        )

        if effective_notfound_percent > thresholds["max_notfound_percent"]:
            issues.append(
                (
                    f"{effective_notfound} non-allowlisted NOTFOUND prefixes "
                    f"({effective_notfound_percent:.1f}% > "
                    f"{thresholds['max_notfound_percent']}%)"
                )
            )
            if risk_level == "low":
                risk_level = "medium"
        ...
        if thresholds["require_vrp_data"] and (
            stats["vrp_entries"] == 0 or stats.get("vrp_data_stale", True)
        ):
        ...
        passed = len(issues) == 0 or (
            risk_level == "medium" and self.config.strictness_level == "low"
        )
    ...
    def _compute_validation_stats(
        self, validation_results: List["RPKIValidationResult"]
    ) -> Dict[str, int]:
        """
