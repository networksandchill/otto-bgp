...
class JuniperSSHCollector:
    ...
    def collect_from_devices(
        ...
        if failed_devices:
            extra_devices = len(failed_devices) - 5
            extra_message = f" (and {extra_devices} more)" if extra_devices > 0 else ""
            failed_summary = ", ".join(failed_devices[:5]) + extra_message
            self.logger.warning(f"Failed devices: {failed_summary}")
        ...
        if show_progress and parallel_results:
            ...
            performance_msg = (
                "Performance: estimated sequential="
                f"{estimated_sequential:.1f}s, actual parallel={actual_duration:.1f}s, "
