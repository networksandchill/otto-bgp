...
class MatrixStatistics:
    ...
    shared_as_numbers: Dict[int, List[str]] = field(default_factory=dict)  # AS -> [routers]
...
class DeploymentMatrix:
    ...
    def generate_router_as_matrix(self, profiles: List[RouterProfile]) -> Dict:
        ...
        for profile in profiles:
            router_data = {
                ...
                "as_count": len(profile.discovered_as_numbers) if profile.discovered_as_numbers else 0,
        ...
        self.logger.info(f"Matrix generated: {stats.total_routers} routers, {stats.total_as_numbers} AS numbers")
        
        return matrix
    
    def _calculate_statistics(self, profiles: List[RouterProfile], as_counter: Dict[int, int]) -> MatrixStatistics:
        ...
        as_counts = []
        for profile in profiles:
            count = len(profile.discovered_as_numbers) if profile.discovered_as_numbers else 0
        ...
        for as_num, count in as_counter.items():
            if count > 1:
                routers = []
                for profile in profiles:
                    if profile.discovered_as_numbers and as_num in profile.discovered_as_numbers:
    ...
    def export_csv(self, matrix: Dict, output_path: Optional[Path] = None) -> Path:
        ...
        with open(output_path, 'w', newline='') as csvfile:
            ...
            # Write header
            writer.writerow(['Router', 'IP Address', 'Site', 'Role', 'AS Count', 'AS Numbers', 'BGP Groups'])
            
            # Write router data
            for hostname, data in matrix["routers"].items():
                as_numbers_str = ', '.join(str(as_num) for as_num in data["as_numbers"])
                bgp_groups_str = ', '.join(data["bgp_groups"].keys()) if data["bgp_groups"] else ""
    ...
    def generate_summary_report(self, matrix: Dict, output_path: Optional[Path] = None) -> Path:
        """
        ...
        with open(output_path, 'w') as report:
            ...
            for hostname, data in matrix["routers"].items():
                ...
                if data['as_numbers']:
                    report.write(f"  AS Numbers: {', '.join(str(as_num) for as_num in data['as_numbers'][:10])}")
            ...
            if stats['shared_as_numbers']:
                ...
                for shared in stats['shared_as_numbers']:
                    report.write(f"AS{shared['as_number']}: {', '.join(shared['routers'])}\n")
...
def generate_deployment_matrix(profiles: List[RouterProfile], output_dir: str = "policies/reports") -> Dict[str, Path]:
    """
