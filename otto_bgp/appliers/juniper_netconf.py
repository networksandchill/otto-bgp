...
from otto_bgp.utils.timeout_config import (
    ExponentialBackoff, TimeoutContext, TimeoutType
)
...
class JuniperPolicyApplier:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None,
        ...
        self.safety_manager = (
            safety_manager or UnifiedSafetyManager(logger=self.logger)
        )
    ...
    def _check_autonomous_mode(self) -> bool:
        ...
        try:
            ...
            return (
                config.autonomous_mode.enabled and
                config.autonomous_mode.notifications.email.enabled
            )
        ...
        while attempt_count <= max_retries:
            try:
                ...
                if not self._is_retryable_netconf_error(e):
                    error_msg = (
                        f"{operation_name} failed with fatal error: {e}"
                    )
                ...
                if not first_failure_notified and self.autonomous_mode:
                    msg = (
                        f"{operation_name} failed (attempt 1), "
                        f"will retry: {e}"
                    )
                    self.logger.warning(msg)
                    try:
    ...
    def _fetch_current_state(self) -> str:
        ...
        try:
            ...
            policy_filter = (
                '<configuration><policy-options/></configuration>'
            )
            policy_config = self.device.rpc.get_config(
                filter_xml=policy_filter,
                options={'format': 'text'}
            )

            bgp_filter = (
                '<configuration><protocols><bgp/></protocols>'
                '</configuration>'
            )
            bgp_config = self.device.rpc.get_config(
                filter_xml=bgp_filter,
                options={'format': 'text'}
            )
    ...
    def connect_to_router(
        ...
        try:
            ...
            error_msg = (
                f"Unexpected error connecting to {hostname}: {e}"
            )
            self.logger.error(error_msg)
    ...
    def load_router_policies(
            self,
            policies_dir: Path) -> List[Dict[str, str]]:
        """
        ...
        self.logger.info(
            f"Loaded {loaded_count} policies from {policies_dir}"
        )
        return policies
    ...
    def preview_changes(
        ...
        try:
            ...
            if not diff:
                ...
                self.logger.info(
                    f"Generated diff with {line_count} lines"
                )
    ...
    def apply_with_confirmation(
        ...
        import os
        from otto_bgp.appliers.mode_manager import (
            ModeManager, CommitInfo
        )
        ...
        try:
            ...
            timeout_msg = (
                f"Initiating confirmed commit "
                f"(timeout: {confirm_timeout}min)"
            )
            ...
            pending_msg = (
                f"Policies applied with confirmation pending "
                f"(ID: {commit_id})"
            )
            ...
            if is_transient:
                ...
                try:
                    ...
                    if commit_retry_guardrail:
                        ...
                        if not guardrail_result.passed:
                            ...
                            if self.autonomous_mode:
                                ...
                                notif = (
                                    self.safety_manager.
                                    send_netconf_event_notification
                                )
                                notif(
                                    'commit',
                                    hostname,
                        ...
                        try:
                            ...
                            if diff:
                                ...
                                success_msg = (
                                    f"Commit succeeded after retry "
                                    f"(ID: {commit_id})"
                                )
                            ...
                            recovery_msg = (
                                f"Commit recovery failed: {retry_error}"
                            )
                    ...
                    self.logger.warning(
                        f"Guardrail check failed: {guardrail_error}"
                    )
    ...
    def _run_health_checks(self, timeout: int = 30):
        ...
        if not self.connected or not self.device:
            ...
            return HealthResult(
                success=False, details=[], error=error_msg
            )

        checks = []
        try:
            # Management interface check
            self.device.rpc.get_interface_information(
                interface_name='fxp0'
            )
    ...
    def _combine_policies_for_load(
            self,
            policies: List[Dict[str, str]]) -> str:
        """
