...
class UnifiedSafetyManager:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Unified Safety Manager initialized - "
        ...
        if self._emergency_override:
            self.logger.critical(
                "EMERGENCY OVERRIDE ACTIVE - Safety constraints relaxed"
            )
        ...
        if config and config.rpki and config.rpki.enabled:
            try:
                ...
                rpki_validator = RPKIValidator(
                    ...
                    if config.rpki.vrp_cache_path
                    else None,
                    allowlist_path=Path(config.rpki.allowlist_path)
                    if config.rpki.allowlist_path
                    else None,
                ...
                rpki_guardrail = RPKIGuardrail(
                    rpki_validator=rpki_validator, logger=self.logger
                )
    ...
    def _emergency_rollback(self):
        ...
        with self._state_lock:
            ...
            self.logger.debug(
                f"Executing {len(callbacks_to_execute)} rollback callbacks in thread {thread_id}"
            )
        ...
        for i, callback in enumerate(callbacks_to_execute):
            try:
                self.logger.debug(
                    f"Executing rollback callback {i + 1}/{len(callbacks_to_execute)}"
                )
                callback()
            except Exception as e:
    ...
    def _determine_unified_risk_level(
        self, errors: List[str], warnings: List[str], guardrail_risks: List[str]
    ) -> str:
    ...
    def _determine_safety_decision(self, errors: List[str], risk_level: str) -> bool:
        ...
        if self._emergency_override:
            self.logger.critical(
                f"EMERGENCY OVERRIDE: Bypassing safety decision - Risk: {risk_level}"
            )
            return True
    ...
    def _generate_unified_recommendation(
        self, safe_to_proceed: bool, risk_level: str, guardrail_count: int
    ) -> str:
        ...
        if self._emergency_override:
            ...
            return (
                f"Review warnings from {guardrail_count} guardrails before proceeding"
            )
        else:
        ...
        with self._state_lock:
            checkpoint = {
                ...
                "guardrails_active": sum(
                    1 for g in self.guardrails.values() if g.is_enabled()
                ),
                "thread_id": threading.current_thread().ident,
            }
    ...
    def _send_emergency_notification(self, result: SafetyCheckResult):
        ...
        try:
            self.logger.critical(
                "CRITICAL SAFETY ISSUE - Sending emergency notification"
            )
            ...
            details = {
                ...
                "guardrail_failures": [
                    r for r in result.guardrail_results if not r.passed
                ],
            }

            # Send notification using existing NETCONF notification system
            self.send_netconf_event_notification(
                "safety_critical", "unified_safety_manager", False, details
            )
    ...
    def validate_policies_before_apply(self, policies: List[Dict]) -> SafetyCheckResult:
        """
        ...
        for name, guardrail in self.guardrails.items():
            if guardrail.is_enabled():
                try:
                    ...
                    self.logger.debug(
                        f"Guardrail {name}: {result.risk_level} - {result.message}"
                    )
        ...
        overall_risk = self._determine_unified_risk_level(
            errors, warnings, guardrail_risk_levels
        )
        ...
        recommended_action = self._generate_unified_recommendation(
            safe_to_proceed, overall_risk, len(guardrail_results)
        )
        ...
        self.logger.info(
            f"UNIFIED SAFETY CHECK COMPLETE - Risk: {overall_risk}, "
    ...
    def should_auto_apply(self, policies: List[Dict]) -> bool:
        ...
        if safety_result.risk_level != "low":
            self.logger.info(
                f"Risk level {safety_result.risk_level} - manual approval required"
            )
            return False
        ...
        self.logger.info(
            f"Auto-apply approved: {prefix_count} prefixes, risk={safety_result.risk_level}, ref_threshold={threshold}"
        )
        return True  # Always apply if autonomous mode enabled and low risk

    def send_netconf_event_notification(
        self,
        event_type: str,
        hostname: str,
        success: bool,
        ...
        try:
            ...
            if config is None:
                ...
                autonomous_config = (
                    ...
                    if hasattr(config, "autonomous_mode")
                    else config.get("autonomous_mode", {})
                )
    ...
    def check_bgp_session_impact(self, diff: str) -> Dict[str, str]:
        ...
        if "replace.*import" in diff or "replace.*export" in diff:
            # Policy changes
            impact["policy"] = (
                "Import/export policy changes - may affect route selection"
            )
        ...
        if "neighbor" in diff:
            ...
            for neighbor in neighbors:
                if "delete" in diff:
                    ...
                    impact[neighbor] = (
                        "Neighbor configuration change - session may reset"
                    )
    ...
    def create_rollback_checkpoint(
        self, device_hostname: str, checkpoint_name: Optional[str] = None
    ) -> str:
        """
    ...
    def monitor_post_application(
        self, metrics: Dict[str, any], baseline: Dict[str, any]
    ) -> bool:
        """
        ...
        if current_sessions < baseline_sessions * 0.9:  # Lost >10% of sessions
            issues.append(
                f"BGP session loss: {baseline_sessions} -> {current_sessions}"
            )
            healthy = False
        ...
        if current_routes < baseline_routes * 0.5:  # Lost >50% of routes
            issues.append(
                f"Significant route loss: {baseline_routes} -> {current_routes}"
            )
            healthy = False
    ...
    def _validate_syntax(self, policies: List[Dict]) -> List[str]:
        ...
        for policy in policies:
            ...
            if content.count("{") != content.count("}"):
                errors.append(
                    f"Unbalanced braces in AS{policy.get('as_number', '?')} policy"
                )

            # Check for proper termination
            if "prefix-list" in content and ";" not in content:
                errors.append(
                    f"Missing semicolons in AS{policy.get('as_number', '?')} policy"
                )
    ...
    def _validate_as_numbers(self, policies: List[Dict]) -> List[str]:
        ...
        for policy in policies:
            ...
            res = policy.get("resource") or (
                f"AS{as_number}" if as_number else "unknown"
            )
    ...
    def _format_netconf_event(
        self, event_type: str, hostname: str, success: bool, details: Dict
    ) -> str:
        """
...
Timestamp: {timestamp}\n"""
        ...
        if event_type == "connect":
            ...
            if success:
                ...
                as_numbers = [
                    p.get("as_number") for p in policies if p.get("as_number")
                ]
...
Rollback Status: {details.get("rollback_status", "N/A")}"""
    ...
    def _send_email(self, email_cfg, subject: str, body: str) -> bool:
        ...
        try:
            ...
            if getattr(email_cfg, "delivery_method", "sendmail") == "sendmail":
                sendmail_path = getattr(
                    email_cfg, "sendmail_path", "/usr/sbin/sendmail"
                )
            ...
            with smtplib.SMTP(
                email_cfg.smtp_server, email_cfg.smtp_port, timeout=10
            ) as server:
                if email_cfg.smtp_use_tls:
                    server.starttls(context=context)
                if getattr(email_cfg, "smtp_username", None):
                    server.login(
                        email_cfg.smtp_username, getattr(email_cfg, "smtp_password", "")
                    )
    ...
    def generate_safety_report(
        self, check_result: SafetyCheckResult, output_file: Optional[str] = None
    ) -> str:
        """
        ...
        lines.append(
            f"Safe to Proceed: {'YES' if check_result.safe_to_proceed else 'NO'}"
        )
        lines.append("")
    ...
    def execute_pipeline(
        ...
        hostname: str,
        mode: str,
        rollout_context: Optional[Dict[str, str]] = None,
    ) -> "ApplicationResult":
        """
        ...
        try:
            ...
            with Device(**device_kwargs) as dev:
                ...
                with Config(dev, mode="exclusive") as cu:
                    self.logger.info(
                        f"Exclusive lock acquired with verified host key: {hostname}"
                    )

                    # GUARDRAIL 1.5: RPKI validation (if enabled)
                    if "rpki_validation" in self.guardrails:
                        guardrail = self.guardrails["rpki_validation"]
                        rpki_result = guardrail.check(
                            {"policies": policies, "operation": "pipeline"}
                        )
                        if not rpki_result.passed:
                            self.logger.error(
                                f"RPKI validation failed: {rpki_result.message}"
                            )
                    ...
                    if not self._pre_commit_guardrails(cu, policies):
                        return ApplicationResult(
                            success=False, exit_code=OttoExitCodes.COMMIT_CHECK_FAILED
                        )
                    ...
                    if not commit_info.success:
                        return ApplicationResult(
                            success=False, exit_code=OttoExitCodes.NETCONF_FAILED
                        )
                    ...
                    if not health_result.success:
                        ...
                        return ApplicationResult(
                            success=False, exit_code=OttoExitCodes.HEALTH_CHECK_FAILED
                        )
                    ...
                    return ApplicationResult(
                        success=True, exit_code=OttoExitCodes.SUCCESS
                    )
    ...
    def _confirmed_commit(self, cu: Config, hold_minutes: int = 5) -> CommitInfo:
        ...
        try:
            ...
            self.logger.info(
                f"Confirmed commit successful: {commit_id} ({hold_minutes}min window)"
            )

            return CommitInfo(
                commit_id=commit_id, timestamp=datetime.now().isoformat(), success=True
            )
        except CommitError as e:
    ...
    def _post_commit_guardrails(
        self, device: Device, timeout: int = 30
    ) -> HealthResult:
        """
        ...
        try:
            ...
            device.rpc.get_interface_information(interface_name="fxp0")
    ...
    def _record_rollout_event(
        self, run_id: Optional[str], event_type: str, payload: Dict[str, Any]
    ) -> None:
