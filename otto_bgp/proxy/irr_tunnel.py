...
class IRRProxyManager:
    ...
    def setup_tunnel(self, tunnel_config: Dict[str, Union[str, int]]) -> TunnelStatus:
        ...
        self.logger.info(f"Setting up IRR tunnel: {tunnel_name} -> {remote_host}:{remote_port}")
    ...
    def wrap_bgpq4_command(self, original_cmd: List[str], irr_server: str = None) -> List[str]:
        """
        ...
        self.logger.debug(f"Modified bgpq4 command to use tunnel {tunnel.name} on port {tunnel.local_port}")
        ...
        for tunnel_name, tunnel in self.tunnels.items():
            ...
            if tunnel_name in self.processes:
                ...
                if process.poll() is not None:
                    ...
                    tunnel.state = TunnelState.FAILED
                    tunnel.error_message = f"SSH process terminated with code {process.returncode}"
    ...
    def _build_ssh_command(self, local_port: int, remote_host: str, remote_port: int) -> List[str]:
        """Build SSH tunnel command with security options"""
        cmd = [
            'ssh',
            '-N',  # No remote command
            '-T',  # No TTY
            '-o', 'StrictHostKeyChecking=yes',  # Require host key verification
            '-o', 'BatchMode=yes',  # No interactive prompts
            '-o', f'ConnectTimeout={self.config.connection_timeout}',
    ...
    def _restart_tunnel(self, tunnel_name: str):
        ...
        self.logger.info(f"Restarting tunnel {tunnel_name} (attempt {tunnel.retry_count})")
