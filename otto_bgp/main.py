...
def emergency_cleanup():
    ...
    logger.info(
        "Emergency cleanup: connections=%d, locks=%d, temp_files=%d",
...
def cmd_collect(args):
    ...
    collector = JuniperSSHCollector(connection_timeout=args.timeout, command_timeout=args.command_timeout)
...
def cmd_policy(args):
    ...
    if getattr(args, "test", False):
        ...
        if success:
            print_success("bgpq4 connectivity test: PASSED")
        else:
            print_error("bgpq4 connectivity test: FAILED", "Check network connectivity and bgpq4 installation")
    ...
    if not as_result.as_numbers and not irr_objects:
        print_error("No inputs found", "Provide ASNs (file) and/or IRR objects (--object/--objects-file)")
        return 1
    ...
    try:
        ...
        if as_list:
            ...
            batch_result = bgpq4.generate_policies_batch(as_list, rpki_status=rpki_status)
        else:
...
def cmd_list(args):
    ...
    try:
        ...
        print("Error: Discovery data not found - run 'otto-bgp discover' first")
        return 1
...
def cmd_apply(args):
    ...
    try:
        ...
        if not policy_dir.exists():
            ...
            print("  Run 'otto-bgp discover' and 'otto-bgp policy' first")
            return 1
...
def cmd_pipeline(args):
    ...
    try:
        ...
        # Determine RPKI availability from config and CLI overrides
        rpki_config = getattr(config, "rpki", None)
        rpki_enabled = bool(rpki_config and rpki_config.enabled and not getattr(args, "no_rpki", False))
        logger.info("RPKI validation %s for pipeline execution", "enabled" if rpki_enabled else "disabled")
        ...
        if use_coordinator:
            ...
            coordinator_config = CoordinatorConfig(
                default_concurrency=getattr(args, "concurrency", 5), enable_events=True, auto_progress_stages=True
            )
            ...
            while True:
                try:
                    ...
                    if _time.time() - start > timeout:
                        logger.warning("Timeout waiting for refresh lock; proceeding without lock")
                        fh.close()
                        return None
                    _time.sleep(0.5)
    ...
    try:
        ...
        if shutil.which("rpki-client"):
            ...
            if result.returncode == 0:
                ...
                return True
            logger.error(f"rpki-client fallback failed: {result.returncode} {result.stderr.strip()}")
        else:
...
def cmd_rpki_check(args):
    ...
    try:
        ...
        if _is_stale_or_missing():
            logger.warning("VRP cache stale or missing; attempting one-shot refresh")
            # up to 2 attempts with short waits
            attempts = 0
            while attempts < 2:
                attempts += 1
                _try_vrp_cache_refresh(cache_path, logger)
                _wait_for_cache_ready(cache_path, logger, max_wait_seconds=45)
                if not _is_stale_or_missing():
                    break
                logger.warning(f"Refresh attempt {attempts} did not reach freshness; retrying")

        # Re-check freshness
        if _is_stale_or_missing():
            # Structured logging for monitoring
            logger.warning(f"RPKI_CACHE_STATUS=stale FAIL_CLOSED={rpki_config.fail_closed} MAX_AGE={max_age}")
            if rpki_config.fail_closed:
                logger.error("VRP cache still stale/missing after refresh; failing closed")
                logger.error("RPKI_CACHE_STATUS=missing")
                print("✗ VRP cache stale or missing; fail-closed is enabled")
                return 1
            else:
                logger.warning("VRP cache stale/missing; proceeding due to fail-open configuration")
                print("! VRP cache stale or missing; proceeding (fail-closed disabled)")
                return 0
        ...
        try:
            ...
            if str(test_result.get("state", "")).lower() in ["valid", "invalid", "notfound"]:
                cache_age = time.time() - cache_path.stat().st_mtime
                # Structured logging for monitoring
                logger.info(f"RPKI_CACHE_STATUS=ok AGE_SECONDS={int(cache_age)} PATH={cache_path}")
                ...
                try:
                    status_dir = Path(rpki_config.vrp_cache_path).parent.parent / "cache"
...
def cmd_rpki_override(args):
    ...
    if args.override_action == "list":
        ...
        logger.info("Retrieved %d RPKI overrides", len(overrides))
        if not overrides:
        ...
        if success:
            ...
            logger.info("Disabled RPKI validation for AS%s", args.as_number)
        else:
            print(f"✗ Failed to disable RPKI for AS{args.as_number}")
            logger.error("Failed to disable RPKI validation for AS%s", args.as_number)
            return OttoExitCodes.DATABASE_ERROR
        ...
        if success:
            ...
            logger.info("Enabled RPKI validation for AS%s", args.as_number)
        else:
            print(f"✗ Failed to enable RPKI for AS{args.as_number}")
            logger.error("Failed to enable RPKI validation for AS%s", args.as_number)
            return OttoExitCodes.DATABASE_ERROR
        ...
        if not history:
            ...
            logger.info("Reporting history for AS%s (%d entries)", args.as_number, len(history))
...
def cmd_notify_email(args):
    ...
    try:
        ...
        safety_mgr = UnifiedSafetyManager(logger=logger, enable_signal_handlers=False)
...
def create_common_flags_parent():
    ...
    verbose_group.add_argument("-v", "--verbose", action="store_true", help="Enable verbose logging")
    verbose_group.add_argument("-q", "--quiet", action="store_true", help="Quiet mode (warnings only)")
    ...
    parent_parser.add_argument("--dev", action="store_true", help="Use Podman for bgpq4 (development mode)")
...
def create_parser():
    ...
    apply_parser.add_argument("--port", type=int, help="NETCONF port (default: 830 or NETCONF_PORT env var)")
    apply_parser.add_argument("--timeout", type=int, default=30, help="Connection timeout in seconds (default: 30)")
    ...
    rpki_check_parser = subparsers.add_parser(
        "rpki-check", help="Validate RPKI cache freshness and structure", parents=[common_flags_parent]
    )
    rpki_check_parser.add_argument("--max-age", type=int, help="Maximum cache age in seconds (overrides config)")

    # RPKI Override Management subcommand
    rpki_override_parser = subparsers.add_parser(
        "rpki-override", help="Manage per-AS RPKI validation overrides", parents=[common_flags_parent]
    )
    rpki_override_subparsers = rpki_override_parser.add_subparsers(dest="override_action")

    # List overrides
    rpki_override_subparsers.add_parser("list", help="List all RPKI overrides")
