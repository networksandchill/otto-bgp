...
class ErrorFormatter:
    ...
    @classmethod
    def format_error(
        cls, error: Union[Exception, OttoError], hide_technical: bool = True
    ) -> str:
        """Format an exception with appropriate level of detail"""
        if isinstance(error, OttoError):
            formatted = cls.format_message(
                error.message, error.severity, error.guidance
            )
        ...
        if isinstance(error, FileNotFoundError):
            ...
            return cls.format_message(
                f"File not found: {message}", ErrorSeverity.ERROR, guidance
            )
        elif isinstance(error, PermissionError):
            guidance = "Check file permissions or run with appropriate privileges"
            return cls.format_message(
                f"Permission denied: {message}", ErrorSeverity.ERROR, guidance
            )
        elif isinstance(error, subprocess.TimeoutExpired):
            guidance = "Check network connectivity or increase timeout value"
            return cls.format_message(
                f"Operation timed out: {message}", ErrorSeverity.ERROR, guidance
            )
        elif isinstance(error, ValueError):
            guidance = "Verify input parameters and try again"
            return cls.format_message(
                f"Invalid input: {message}", ErrorSeverity.ERROR, guidance
            )
        elif isinstance(error, KeyboardInterrupt):
            return cls.format_message(
                "Operation interrupted by user", ErrorSeverity.WARNING
            )
        else:
            if hide_technical:
                return cls.format_message(
                ...
                return cls.format_message(
                    f"Unexpected {error_type}: {message}", ErrorSeverity.ERROR
                )
...
class ParameterValidator:
    ...
    def validate_timeout(timeout: int, parameter_name: str = "timeout") -> int:
        ...
        if timeout > 3600:  # 1 hour
            ...
            logger.warning(
                f"Very high timeout ({timeout}s) - this may cause long delays"
            )
    ...
    def validate_file_exists(
        file_path: Union[str, Path], parameter_name: str = "file"
    ) -> Path:
    ...
    def validate_directory_writable(
        dir_path: Union[str, Path], parameter_name: str = "directory"
    ) -> Path:
    ...
    @staticmethod
    def validate_as_number(
        as_number: Union[str, int], parameter_name: str = "as_number"
    ) -> int:
...
def validate_common_args(args):
    ...
    if hasattr(args, "command_timeout") and args.command_timeout is not None:
        args.command_timeout = validator.validate_timeout(
            args.command_timeout, "command_timeout"
        )

    if hasattr(args, "confirm_timeout") and args.confirm_timeout is not None:
        args.confirm_timeout = validator.validate_timeout(
            args.confirm_timeout, "confirm_timeout"
        )
