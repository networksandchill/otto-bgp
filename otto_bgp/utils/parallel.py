...
class ParallelExecutor:
        ...
        with self._managed_thread_pool() as executor:
            ...
            try:
                ...
                for future in as_completed(future_to_item):
                    if self._shutdown_requested:
                        self.logger.warning(
                            "Shutdown requested during execution, terminating batch"
                        )
                        break
                    ...
                    try:
                        ...
                        result = ParallelResult(
                            item=item, success=False, error=str(e), duration=0.0
                        )
    ...
    def _execute_task(self, task_func: Callable, item: Any, **kwargs) -> ParallelResult:
        ...
        try:
            ...
            return ParallelResult(
                item=item, success=True, result=result, duration=duration
            )
            ...
            return ParallelResult(
                item=item, success=False, error=str(e), duration=duration
            )
    ...
    def _cleanup_all_executors(self):
        ...
        with self._cleanup_lock:
            ...
            self.logger.info(
                f"Cleaning up {len(self._active_executors)} active executors"
            )
    ...
    def _show_progress(self, completed: int, total: int, last_success: bool):
        ...
        print(
            f"\r[{bar}] {percentage:.1f}% ({completed}/{total}) {status}",
            end="",
            flush=True,
        )


def parallel_discover_routers(
    devices: List, collector, inspector, max_workers: Optional[int] = None
) -> Tuple[List, List]:
    """
    ...
    logger.info(
        f"Parallel workers selected: {max_workers} (items={len(devices)}, cpus={os.cpu_count()})"
    )
    ...
    results = executor.execute_batch(
        items=devices, task_func=discover_single_router, task_name="Discovering routers"
    )
...
def parallel_generate_policies(
    as_numbers: List[int], wrapper, max_workers: Optional[int] = None
) -> List:
    """
    ...
    logger.info(
        f"Parallel workers selected: {max_workers} (items={len(as_numbers)}, cpus={os.cpu_count()})"
    )
    executor = ParallelExecutor(max_workers=max_workers)
