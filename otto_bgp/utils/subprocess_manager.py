...
class ManagedProcess:
    ...
    def __enter__(self) -> 'ManagedProcess':
        ...
        try:
            ...
            self.logger.debug(f"Started process {self.process.pid}: {' '.join(self.command)}")
    ...
    def wait_for_completion(self) -> ProcessResult:
        ...
        try:
            ...
            self.logger.warning(f"Process {self.process.pid} timeout after {self.timeout}s")
    ...
    def terminate_gracefully(self, timeout: int = 5):
        ...
        try:
            ...
            # Force kill if graceful termination fails
            self.logger.warning(f"Force killing unresponsive process {self.process.pid}")
            self.process.kill()
            self.process.wait()
...
class ProcessRegistry:
    ...
    def register_process(self, pid: int, process: subprocess.Popen):
        ...
        with self.process_lock:
            ...
            self.logger.debug(f"Registered process {pid} (total active: {len(self.active_processes)})")
    ...
    def unregister_process(self, pid: int):
        ...
        with self.process_lock:
            if pid in self.active_processes:
                ...
                self.logger.debug(f"Unregistered process {pid} (total active: {len(self.active_processes)})")
    ...
    def cleanup_all_processes(self):
        ...
        with self.process_lock:
            ...
            self.logger.warning(f"Emergency cleanup of {len(self.active_processes)} active processes")
                ...
                try:
                    if process.poll() is None:
                        ...
                        try:
                            ...
                            self.logger.warning(f"Force killing unresponsive process {pid}")
                            process.kill()
                            process.wait()
    ...
    def _signal_handler(self, signum, frame):
        ...
        self.logger.info(f"Received signal {signum}, cleaning up {len(self.active_processes)} processes")
...
def run_with_resource_management(command: List[str], 
                                timeout: Optional[int] = None,
