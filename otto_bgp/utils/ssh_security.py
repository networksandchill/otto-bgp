    ...
    def __init__(self, known_hosts_path: Optional[str] = None, strict: bool = True):
        ...
        if self.known_hosts_path.exists():
            try:
                ...
                self.logger.info(f"Loaded {len(self.host_keys)} host keys from {self.known_hosts_path}")
            except Exception as e:
                self.logger.error(f"Failed to load known_hosts from {self.known_hosts_path}: {e}")
                if self.strict:
                    raise RuntimeError(f"Cannot load known_hosts file in strict mode: {e}")
        else:
            if self.strict:
                raise RuntimeError(
    ...
    def missing_host_key(self, client, hostname, key):
        ...
        if hostname in self.host_keys:
            ...
            for known_key in self.host_keys[hostname].values():
                if known_key.get_fingerprint() == key.get_fingerprint():
                    ...
                    self.logger.debug(f"Host {hostname} key verified: {key_fingerprint}")
                    return
        ...
        if self.strict:
            ...
            try:
                ...
                self.logger.info(f"Host key for {hostname} saved to {self.known_hosts_path}")
            except Exception as e:
